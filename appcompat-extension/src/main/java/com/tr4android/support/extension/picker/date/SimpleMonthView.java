...
class SimpleMonthView extends View {
    private static final int VIEW_STATE_SELECTED = 1;
    private static final int VIEW_STATE_PRESSED = 2;
    ...
    public CharSequence getTitle() {
        if (mTitle == null) {
            mTitle = DateFormatUtils.format(mTitleFormatter, mCalendar);
        }
        return mTitle;
    }
    ...
    void setDaySelectorColor(ColorStateList dayBackgroundColor) {
        final int selectedColor = dayBackgroundColor.getColorForState(
    ...
    void setDayHighlightColor(ColorStateList dayHighlightColor) {
        final int pressedColor = dayHighlightColor.getColorForState(
                new int[]{android.R.attr.state_enabled, android.R.attr.state_pressed}, 0);
    ...
    private String getDayOfWeekLabel(int dayOfWeek) {
        ...
        return DateFormatUtils.formatDayOfWeek(mDayOfWeekFormatter, mDayOfWeekLabelCalendar);
    }
    ...
    private static int[] buildState(boolean enabled, int state) {
        // Compute the size of the resulting stateSet
        int size = (enabled ? 1 : 0) + (state != 0 ? 1 : 0);
        int[] stateSet = new int[size];
        if (enabled) stateSet[0] = android.R.attr.state_enabled;
        if (state == VIEW_STATE_SELECTED) {
            stateSet[size - 1] = android.R.attr.state_selected;
        } else if (state == VIEW_STATE_PRESSED) {
            stateSet[size - 1] = android.R.attr.state_pressed;
        }
        return stateSet;
    }
